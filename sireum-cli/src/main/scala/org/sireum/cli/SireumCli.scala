...
""".trim) 
  ...
  if (i == args.length) {
    ...
    result.featureName = "Sireum Tools"
    val keys = List[String]("-h", "--help", "-gcn", "--generated-class-name", "-d", "--directory", "-ts", "--type-substitutions")
    var j = i
    var k = -1
    val seenopts = scala.collection.mutable.ListBuffer.empty[String]
...
""".trim) 
  ...
  if (i == args.length) {
    ...
    result.featureName = "Sireum Tools"
    val keys = List[String]("-h", "--help", "-p", "--package", "-d", "--directory", "-cp", "--classpath", "-c", "--class-name")
    var j = i
    var k = -1
    val seenopts = scala.collection.mutable.ListBuffer.empty[String]
...
""".trim
    ...
    parseModeHelper("tools", Seq("cligen", "jvm", "pipeline", "antlr", "sapper", "upickler", "jsgen"), args, i) {
      _ match {
        case "cligen" =>
...
""".trim) 
  ...
  if (i == args.length) {
    ...
    result.featureName = "Sireum Amandroid Cli:Amandroid.sapp"
    val keys = List[String]("-h", "--help", "-m", "--memory", "-msg", "--message", "-t", "--type", "-p", "--parallel", "-o", "--outdir", "-to", "--timeout", "-ns", "--nostatic", "-ni", "--no-icc", "-k", "--k-context")
    var j = i
    var k = -1
    val seenopts = scala.collection.mutable.ListBuffer.empty[String]
...
-h | --help
-f  | --format     Graph output format. [Default: GraphML, Choices: (GML, DOT,
                   GraphML)]
...
""".trim) 
  ...
  if (i == args.length) {
    ...
    result.featureName = "Sireum Amandroid Cli:Amandroid.sapp"
    val keys = List[String]("-h", "--help", "-f", "--format", "-gt", "--graph-type", "-m", "--memory", "-msg", "--message", "-t", "--type", "-p", "--parallel", "-o", "--outdir", "-to", "--timeout", "-ns", "--nostatic", "-ni", "--no-icc", "-k", "--k-context")
    var j = i
    var k = -1
    val seenopts = scala.collection.mutable.ListBuffer.empty[String]
