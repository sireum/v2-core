...
object SireumDistro extends App {
  ...
  def parseDistroArgs(i : Int) {
      def notMode(mode : String, parentMode : String) {
        err.println(mode + " is not a mode of " + parentMode)
        err.flush
      }
    val last = i + 1 == args.length
    val strLe = { (s1 : String, s2 : String) => s1 <= s2 }
    val appModes = Seq("distro", "launch", "tools")
    val distroModes = Seq("clean", "install", "list", "uninstall", "update", "version")
    val mode = args(i)
    ...
    if (modeMatches.size == 1) {
      modeMatches(0) match {
        ...
        case "install" =>
          if (last) {
            err.println("Please specify features to install")
            err.flush
          } else {
            args(i + 1) match {
              case "-d" =>
                if (args.length > i + 3) {
                  ...
                  if (!unmanagedDir.exists) {
                    err.println(unmanagedDir.getAbsolutePath + " does not exist")
                    err.flush
                  } else if (!unmanagedDir.isDirectory) {
                    err.println(unmanagedDir.getAbsolutePath + " is not a directory")
                    err.flush
                  } else
                    install(args.slice(i + 3, args.length) : _*)
                } else if (args.length == i + 2) {
                  err.println("Missing install option -d argument")
                  err.flush
        ...
        case "uninstall" =>
          if (last) {
            err.println("Please specify features to uninstall")
            err.flush
          } else {
      ...
      out.println("Did you mean one of the following modes?")
      for (mm <- modeMatches) {
        out.println("  " + mm)
      }
      out.flush
