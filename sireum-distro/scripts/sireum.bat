...
object SireumDistro extends App {
  ...
  uninstall <feature> <name>   Uninstall a feature and all features 
                               depending on it
...
""".trim)
  ...
  def uninstall(featureName : String) {
    ...
    if (all) {
      ...
      return
    } else if (!installedFeatures.contains(featureName))
      return

    val features = getFeatures
    var seenFeatures = Set[String]()
    val featuresToDelete = ArrayBuffer[String](featureName)
          ...
          for (featureToDelete <- featuresToDelete)
            if (featureToDelete.endsWith(SAPP_EXT)) {
              val featureRelPath = "apps/" +
                featureToDelete.
                substring(0, featureToDelete.length - SAPP_EXT.length).
                toLowerCase
        ...
        for (f <- installedFeatures)
          if (features.contains(f))
            ...
            if (f.endsWith(SAPP_EXT)) {
              val file = new File(sireumDir, "apps/" + f.substring(0, f.length - SAPP_EXT.length))
  ...
  def getAppPath(filePath : String) = {
    ...
    appPath = appPath.substring(0, appPath.length - SAPP_EXT.length)
