...
--   type BufferType is array (PartitionID) of MsgType;
   type BufferType is array (PartitionID) of Integer;
...
--   NullMsg: constant MsgType := MsgType'(0, 0, PayloadType'(0,0,0,0,0));
   ...
   procedure ProcessOnePartition(Partition: in PartitionID; 
				 ...
				 NullMsg0: in Integer;
				 K: in PartitionID; X: out Integer);
   
   --# global in out InBox0, InBox1;
   --# derives OutBox from OutBox, CommPolicy, NullMsg0 &
   --#         InBox0 from InBox0, Partition, OutBox, CommPolicy, NullMsg0 &
   --#         InBox1 from InBox1, Partition, OutBox, CommPolicy, NullMsg0 &
   --#         X      from InBox1, Partition, OutBox, CommPolicy, NullMsg0, K;
