...
trait ControlFlowGraph[VirtualLabel]
  ...
  def successor(node : ControlFlowGraph.Node, branch : Option[Branch]) : ControlFlowGraph.Node
  def predecessor(node : ControlFlowGraph.Node, branch : Option[Branch]) : ControlFlowGraph.Node
}
